                ...
                <c:choose>
	                <c:when test="${eventCRF.stage.initialDE}">
                        <img src="images/icon_InitialDE.gif"
                             ...
                             title="<fmt:message key="initial_data_entry" bundle="${resword}"/> + Hello">
	                </c:when>
		...
		<c:if test="${! empty formMessages}">
			...
			<div id="errorMessagesContainer" class="aka_err_message">
				<ul>
                    ...
                    <label onclick="getFocused('<c:out value="${formMsg.key}"/>');"><c:out
		...
		<table class="table">
			<tr>
				...
				<script type="text/JavaScript" language="JavaScript">
                    ...
                    function sm(obl, chkbox, wd, ht) {
                        if (chkbox.checked == false) {
                            checkboxObject = chkbox;
                            return;
                        }
                        checkboxObject = chkbox;
                        var h = 'hidden';
							...
							<td valign="top">
								...
								<c:forEach var="formMsg" items="${formMessages}">
									<c:set var="inputValue">input<c:out
											value="${displayItem.singleItem.item.id}"/></c:set>
									<c:if test="${formMsg.key eq inputValue}">
									...
									<tr>
										<c:choose>
											<c:when test="${stage !='adminEdit' && section.lastSection}">
												<c:choose>
													<c:when test="${section.eventDefinitionCRF.electronicSignature == true}">
														<td valign="bottom"><input type="checkbox" id="markCompleteId"
										...
										<%--尾部保存与退出按钮 2017-5-14 修改 引入bootstrap按钮样式--%>
										<td>
											<button type="submit" id="srl" disabled name="submittedResume"
											        class="btn btn-primary"
											        onclick="disableSubmit(); this.form.submit();">
										...
										<td>
											<button type="submit" id="sel" disabled name="submittedExit" value=""
											        class="btn btn-default"
											        onClick="return checkEntryStatus('DataStatus_bottom');">
